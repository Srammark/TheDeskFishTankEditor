...
import { EMMarshalledID } from '../Json_DLL/MarshalledID';
import Path from './Path';
import PlatformInfo, { EMPlatform } from '../Platform_DLL/PlatformInfo';
...
export class URI implements UriComponents {
	...
	public static File(path: string): URI
	{
		...
		if (PlatformInfo.Platform === EMPlatform.Windows) {
	 ...
	 */
	public static JoinPath(uri: URI, ...pathFragment: string[]): URI
	{
		if (!uri.path) {
			throw new Error(`[UriError]: cannot call joinPath on URI without path`);
		}
		let newPath: string;
		if (PlatformInfo.Platform === EMPlatform.Windows && uri.scheme === 'file') {
			newPath = URI.File(Path.Join(UriToFsPath(uri, true), ...pathFragment)).path;
		} else {
	 ...
	 */
	public static Revive(data: UriComponents | URI): URI;
	public static Revive(data: UriComponents | URI | undefined): URI | undefined;
	public static Revive(data: UriComponents | URI | null): URI | null;
	public static Revive(data: UriComponents | URI | undefined | null): URI | undefined | null;
	public static Revive(data: UriComponents | URI | undefined | null): URI | undefined | null
	{
		if (!data) {
			return data;
...
interface UriState extends UriComponents {
	$mid: EMMarshalledID.Uri;
	external?: string;
	fsPath?: string;
	_sep?: 1;
}

const _pathSepMarker = PlatformInfo.Platform === EMPlatform.Windows ? 1 : undefined;
