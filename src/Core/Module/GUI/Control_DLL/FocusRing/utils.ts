...
import { DimensionPosition, type Dimensions } from './types';
...
{
    const inPx = Object.values(DimensionPosition).reduce((acc, key) => ({ ...acc, [key]: dimensions[key] + 'px' }), {});
...
export const setPosition = (
    ...
    const scrollTop = onBody ? window.scrollY : Math.floor(root.value.scrollTop) - Math.ceil(cRect.top) - 1

    const scrollLeft = onBody ? window.scrollX : Math.floor(root.value.scrollLeft) - Math.ceil(cRect.left) - 1
